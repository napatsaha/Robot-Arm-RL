# -*- coding: utf-8 -*-
"""
Created on Wed May 17 15:58:12 2023
...
class DSACAgent:
    ...
    def _initialise_model(self):
        # Critic networks
        self.critic1 = CriticNetwork(self.n_states, self.n_actions, self.n_branches, self.hidden_size)
        self.critic2 = CriticNetwork(self.n_states, self.n_actions, self.n_branches, self.hidden_size)
        
        # Target Critic as exact copies
        self.target_critic1 = CriticNetwork(self.n_states, self.n_actions, self.n_branches, self.hidden_size)
        self.target_critic2 = CriticNetwork(self.n_states, self.n_actions, self.n_branches, self.hidden_size)
    ...
    def critic_loss(self, states, actions, rewards, nextstates, done):
        ...
        with torch.no_grad():
            probs, log_probs = self.actor(nextstates)
            next_q1 = self.target_critic1(nextstates)
            next_q2 = self.target_critic2(nextstates)
            min_next_q = torch.min(next_q1, next_q2)
            soft_state = probs * (min_next_q - self.alpha * log_probs)
            soft_state = soft_state.sum(dim=-1)#.unsqueeze(-1)
            target_q = rewards + (1 - done) * self.gamma * soft_state
            target_q = target_q.sum(dim=-1) # AGG FUNC HERE
            
        pred_q1 = self.critic1.gather(states, actions).squeeze(-1).sum(dim=-1) # AGG FUNC HERE
        pred_q2 = self.critic2.gather(states, actions).squeeze(-1).sum(dim=-1) # AGG FUNC HERE
