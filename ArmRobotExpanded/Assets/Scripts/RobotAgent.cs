...
public class RobotAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        {
            // int actionValue = (int)(actions.ContinuousActions[i] - 1); // Turns 0,1,2 to -1,0,1 respectively
            // RotationDirection direction = (RotationDirection)(actionValue); // Convert to enumerator
            // controller.RotateJoint(i, direction);
            float actionValue = actions.ContinuousActions[i];
            if (Mathf.Abs(actionValue) > Mathf.Epsilon)
            {
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        for (int i = 0; i < actions.Length; i++)
        {
            ...
            // Debug.Log("Joint ID: "+i+"  Input Value: "+inputVal);
